...
type DNSResolverImpl struct {
	...
	txtStats                 metrics.Scoped
	aStats                   metrics.Scoped
	caaStats                 metrics.Scoped
	mxStats                  metrics.Scoped
}
...
func NewDNSResolverImpl(readTimeout time.Duration, servers []string, stats metrics.Scoped) *DNSResolverImpl {
...
func NewTestDNSResolverImpl(readTimeout time.Duration, servers []string, stats metrics.Scoped) *DNSResolverImpl {
...
func (dnsResolver *DNSResolverImpl) exchangeOne(hostname string, qtype uint16, msgStats metrics.Scoped) (rsp *dns.Msg, err error) {
	m := new(dns.Msg)
	// Set question type
