...
func newGoogleSafeBrowsing(gsb *cmd.GoogleSafeBrowsingConfig) va.SafeBrowsing {
	...
	if gsb.APIKey == "" {
		cmd.FailOnError(errors.New("seriously"), "a Google Safe Browsing config was given but it did not include a Google API key in APIKey")
	}
	if gsb.DataDir == "" {
		cmd.FailOnError(errors.New("seriously"), "a Google Safe Browsing config was given but it did not include data directory to store the hashes file in DataDir")
	}
